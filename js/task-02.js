...
function imgBigChange(elem) {
  ...
  document
    .querySelector(".buton-task-two-negativ")
    .addEventListener("click", () => {
      imgNumber = 0;
      document.querySelector(".task2-div").remove();
    });
  document
    .querySelector(".buton-task-two-left")
    .addEventListener("click", () => {
      imgGooDawn();
      ...
      document.querySelector(
        ".modal-img-text"
      ).textContent = `${images[imgNumber].alt}`;
    });
  document
    .querySelector(".buton-task-two-rait")
    .addEventListener("click", () => {
      imgGooUp();
      let imgChange = document.querySelector(".task2-ImgSt2");
      imgChange.setAttribute("src", `${images[imgNumber].url}`);
...
document
  .querySelector("img[focus-atribut1]")
  .addEventListener("focus", (event) => {
    imgBigChange(event.target.alt);
  });
document
  .querySelector("img[focus-atribut2]")
  .addEventListener("focus", (event) => {
    imgBigChange(event.target.alt);
  });
document
  .querySelector("img[focus-atribut3]")
  .addEventListener("focus", (event) => {
    imgBigChange(event.target.alt);
  });
document
  .querySelector("img[focus-atribut4]")
  .addEventListener("focus", (event) => {
    imgBigChange(event.target.alt);
  });
document
  .querySelector("img[focus-atribut5]")
  .addEventListener("focus", (event) => {
    imgBigChange(event.target.alt);
  });
